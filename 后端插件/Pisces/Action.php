...
class Pisces_Action extends Typecho_Widget implements Widget_Interface_Do {
    private $db;
    private $res;
    ...
    public function __construct($request, $response, $params = NULL) {
        ...
        $this->apisecret = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->apiSecret;
        $this->appid = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->appid;
        $this->appsecret = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->appsecret;
        $this->wxappid = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->wxappid;
        $this->wxappsecret = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->wxappsecret;
        $swipe = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->swipePosts;
    ...
    private function posts() {
        ...
        $temp = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->hiddenShare;
        $author = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->author;
        $website = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->website;
        $account = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->account;
        $thumb = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->thumb;
        $result = array();
        foreach ($posts as $post) {
            ...
            $post['thumb'] = $this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $post['cid'])->where('name = ? ',thumb))?$this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $post['cid'])->where('name = ? ',thumb)):array(array("str_value"=>$thumb));
            $post['description'] = $this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $post['cid'])->where('name = ? ', description))?$this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $post['cid'])->where('name = ? ', description)):array(array("str_value"=>"没有配置描述，点进去看看不就知道了。"));
    ...
    private function getaboutcid()
    {
       $sec = self::GET('apisec', 'null');
        ...
        $cid = 'none';
        $cid = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->aboutCid; 
    ...
    private function getcat()
    {
        ...
        $sec = self::GET('apisec', 'null');
        self::checkApisec($sec);
        $temp = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->hiddenmid;                
    ...
    //首页热门分类
        private function gethotcat()
    {
       $sec = self::GET('apisec', 'null');
        self::checkApisec($sec);
        $hotcat='none';
        $hotcat = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->hotcat; 
        $this->export($hotcat);
    }
    ...
    private function likePost()
    {
        ...
        if(self::checkliked($openid,$cid)) {
            ...
            $this->db->query($this->db->delete('table.pisceslike')->rows(array('openid' => $openid, 'cid' => $cid))->where('openid =?', $openid)->where('cid =?', $cid));
            $status = 'dislike';
        }
        else {
            $this->db->query($this->db->update('table.contents')->rows(array('likes' => (int)$row['likes']+1))->where('cid = ?', $cid));
            //更新赞数据库
            $this->db->query($this->db->insert('table.pisceslike')->rows(array('openid' => $openid, 'cid' => $cid)));
            $status = 'like';
        }
    ...
    private function getuserlikedinfo()
    {
        ...
        $row = $this->db->fetchRow($this->db->select('openid','cid')->from('table.pisceslike')->where('cid = ?', $cid)->where('openid = ?', $openid));
        if(self::checkliked($openid,$cid))
        {
            //已存在该用户点赞
            $this->export('true');
    ...
    private function getuserlikedlist()
    {
        ...
        if($cid != 'null')
        {
            $openids = $this->db->fetchAll($this->db->select('openid')->from('table.pisceslike')->where('cid = ?', $cid));
            foreach($openids as $openid) {
                $temp = $this->db->fetchAll($this->db->select('nickname','avatarUrl')->from('table.pisces')->where('openid = ?', $openid));                
                if(sizeof($temp)>0) {
                    $likeinfo[] = $temp[0];
                }
            }
    ...
    private function getusercollection()
    {
        ...
        if($openid != 'null')
        {
            $cid = $this->db->fetchAll($this->db->select('cid')->from('table.pisceslike')->where('openid = ?', $openid));
            if(sizeof($cid)>0){
                 foreach($cid as $cid){
                    $post = $this->db->fetchAll($this->db->select('cid', 'title',  'type', 'slug')->from('table.contents')->where('cid = ?', $cid)->where('type = ?', 'post')->where('status = ?', 'publish'));
    ...
    private function getfriend()
    {
        ...
        self::checkApisec($sec);
            ...
            $openids = $this->db->fetchAll($this->db->select('openid')->from('table.pisces'));
            $result = array();
            
            foreach($openids as $openids) {
                $friend = $this->db->fetchAll($this->db->select('id','openid','nickname','avatarUrl')->from('table.pisces'));
            }
            $this->export($friend);
        ...
        if($code != 'null')
        {
            ...
            if( $openid != null && $openid != '' ) {
                ...
                if(sizeof($row)>0) {
                    ...
                    //新用户
                    $this->db->query($this->db->insert('table.pisces')->rows(array('openid' => $openid, 'createtime' => time(), 'lastlogin' => time(), 'nickname' => $nickname, 'avatarUrl' => $avatarUrl, 'city' => $city, 'country' => $country,'gender' => $gender, 'province' => $province, 'type'=>'QQ')));
                    $this->export($openid);
                }
            } else {
                $this->export('none');
            ...
            if( $openid != null && $openid != '' ) {
                ...
                if(sizeof($row)>0) {
                    ...
                    $this->db->query($this->db->insert('table.pisces')->rows(array('openid' => $openid, 'createtime' => time(), 'lastlogin' => time(),
                        ...
                        'gender' => $gender, 'province' => $province, 'type'=>'微信','points'=>0)));
                    $this->export($openid);
                }
            } else {
                $this->export('none');
    ...
    private function getpostbymid()
    {
        ...
        if($mid == 99999999) {
            ...
            foreach($posts as $post) {
                ...
                if(sizeof($temp)>0) {
                $temp['0']['thumb'] = $this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $post['cid'])->where('name = ? ',thumb))?$this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $post['cid'])->where('name = ? ',thumb)):array(array("str_value"=>$thumb));
                $temp['fields'] = $this->db->fetchAll($this->db->select('name','str_value')->from('table.fields')->where('cid = ?', $post['cid']));
                $temp['0']['description'] = $this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $post['cid'])->where('name = ? ', description))?$this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $post['cid'])->where('name = ? ', description)):array(array("str_value"=>"没有配置描述，点进去看看不就知道了。"));
            ...
            if(sizeof($cids)>0){
                foreach($cids as $cid) {
                    ...
                    if(sizeof($post)>0 && $post[0]!=null) {
                        ...
                        $post[0]['thumb'] =  $this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $cid)->where('name = ? ',thumb))?$this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $cid)->where('name = ? ',thumb)):array(array("str_value"=>$thumb));
                        $post['0']['description'] = $this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $cid)->where('name = ? ', description))?$this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('cid = ?', $cid)->where('name = ? ', description)):array(array("str_value"=>"没有配置描述，点进去看看不就知道了。"));
        ...
        if(sizeof($cids)>0){
            foreach($cids as $cid) {
                ...
                if(sizeof($post)>0 && $post[0]!=null) {
                    $post[0]        = $this->widget("Widget_Abstract_Contents")->push($post[0]);                  
                    $post[0]['tag'] = $this->db->fetchAll($this->db->select('name')->from('table.metas')->join('table.relationships', 'table.metas.mid = table.relationships.mid', Typecho_DB::LEFT_JOIN)->where('table.relationships.cid = ?', $cid)->where('table.metas.type = ?', 'tag'));
                    $post[0]['thumb'] = $this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('table.fields.cid = ?', $cid)->where('name = ? ',thumb))?$this->db->fetchAll($this->db->select('str_value')->from('table.fields')->where('table.fields.cid = ?',$cid)->where('name = ? ',thumb)):array(array("str_value"=>$thumb));
                    $post[0]['views'] = $this->db->fetchAll($this->db->select('views')->from('table.contents')->where('table.contents.cid = ?', $cid));
                    $post[0]['likes'] = $this->db->fetchAll($this->db->select('likes')->from('table.contents')->where('table.contents.cid = ?', $cid));
                    $result[]    = $post[0];
                }
    ...
    private function getrecommend()
    {
        ...
        $rec = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->recommend;
        $thumb = Typecho_Widget::widget('Widget_Options')->plugin('Pisces')->thumb;
        $cids = explode(",",$rec);
        $result = array();
        if(sizeof($cids)>0){
